...
<!-- Model Comparison Info Table -->
<xsl:text>| Base (A) | Head (B) | Compared |&#10;</xsl:text>
<xsl:text>| :--- | :--- | :--- |&#10;</xsl:text>
<xsl:text>| **</xsl:text>
<xsl:value-of select="cr:fileDetailsA/@title"/>
<xsl:text>**</xsl:text>
<xsl:if test="cr:fileDetailsA/@isInVcs = 'true' and normalize-space(cr:fileDetailsA/@commitRevision) != ''">
  <xsl:text> (`</xsl:text>
  <xsl:value-of select="substring(cr:fileDetailsA/@commitRevision, 1, 7)"/>
  <xsl:text>`)</xsl:text>
</xsl:if>
<xsl:text> | **</xsl:text>
<xsl:value-of select="cr:fileDetailsB/@title"/>
<xsl:text>**</xsl:text>
<xsl:if test="cr:fileDetailsB/@isInVcs = 'true' and normalize-space(cr:fileDetailsB/@commitRevision) != ''">
  <xsl:text> (`</xsl:text>
  <xsl:value-of select="substring(cr:fileDetailsB/@commitRevision, 1, 7)"/>
  <xsl:text>`)</xsl:text>
</xsl:if>
<xsl:text> | </xsl:text>
...
<xsl:if test="cr:fileDetailsB/@commitAuthor != ''">
  <xsl:text> by </xsl:text>
  <xsl:choose>
    <xsl:when test="contains(cr:fileDetailsB/@commitAuthor, ' &lt;')">
      <xsl:value-of select="substring-before(cr:fileDetailsB/@commitAuthor, ' &lt;')"/>
    </xsl:when>
    <xsl:when test="contains(cr:fileDetailsB/@commitAuthor, ' (')">
      <xsl:value-of select="substring-before(cr:fileDetailsB/@commitAuthor, ' (')"/>
    </xsl:when>
    <xsl:otherwise>
      <xsl:value-of select="cr:fileDetailsB/@commitAuthor"/>
    </xsl:otherwise>
  </xsl:choose>
</xsl:if>
<xsl:text> |&#10;&#10;</xsl:text>
